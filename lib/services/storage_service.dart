...
class StorageService {
  ...
  Future<Directory> _getHistoryDir() async {
    ...
    final historyDir = Directory('${dir.path}/${AppConstants.historyFolderName}');
  ...
  Future<File> _getFileForDate(DateTime date) async {
    ...
    final filename = DateFormat(_dateFormat).format(date) + AppConstants.fileExtension;
  ...
  Future<DayHistory> loadDayHistory(DateTime date) async {
    ...
    for (int i = 0; i < lines.length; i++) {
      try {
        final event = Event.fromFileLine(lines[i], 'event_${date.millisecondsSinceEpoch}_$i');
        events.add(event);
      } catch (e) {
  ...
  Future<void> updateEvent(DateTime originalDate, Event updatedEvent) async {
    ...
    final newDate = DateTime(updatedEvent.start.year, updatedEvent.start.month, updatedEvent.start.day);
    
    // If date changed, move event to new file
    if (originalDate != newDate) {
      // Remove from original file
      final updatedOriginalHistory = originalDayHistory.removeEvent(updatedEvent.id);
  ...
  Future<List<DateTime>> getAvailableDates() async {
    try {
      final historyDir = await _getHistoryDir();
      final files = await historyDir.list().where((entity) => 
